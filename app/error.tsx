  ...
  return (
    <main className="flex-1 w-full max-w-3xl mx-auto px-4 py-20">
      <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center dark:border-slate-800 dark:bg-slate-900">
        ...
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">页面出错了</h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
          抱歉，页面渲染时遇到了问题，你可以重试或返回首页。
        </p>
        {error.digest && (
          <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">错误码：{error.digest}</p>
        )}
